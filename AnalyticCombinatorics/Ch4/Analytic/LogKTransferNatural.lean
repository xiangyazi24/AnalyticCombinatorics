...
/-- `n^{α-1} = o(n^{α-1}·(log n)^k)` for `k ≥ 1`. -/
theorem algebraic_scale_isLittleO_logK_scale {α : ℝ} {k : ℕ} (hk : 1 ≤ k) :
    (fun n : ℕ => (n : ℝ) ^ (α - 1))
      =o[atTop] (fun n : ℕ => (n : ℝ) ^ (α - 1) * (Real.log n) ^ k) := by
  have hlog_atTop : Tendsto (fun n : ℕ => Real.log n) atTop atTop :=
    Real.tendsto_log_atTop.comp tendsto_natCast_atTop_atTop
  have hk_atTop : Tendsto (fun n : ℕ => (Real.log n) ^ k) atTop atTop :=
    (tendsto_pow_atTop (by omega : k ≠ 0)).comp hlog_atTop
  have hlog : (fun _ : ℕ => (1 : ℝ)) =o[atTop] (fun n : ℕ => (Real.log n) ^ k) :=
    (isLittleO_one_left_iff ℝ).mpr (tendsto_norm_atTop_atTop.comp hk_atTop)
  have h := (Asymptotics.isBigO_refl (fun n : ℕ => (n : ℝ) ^ (α - 1)) atTop).mul_isLittleO hlog
  simpa using h

/-- **Strong-remainder general logᵏ transfer** (`k ≥ 1`). -/
theorem logK_transfer_theorem_strong_remainder
    {α : ℝ} {C : ℂ} {R φ : ℝ} {f : ℂ → ℂ} {p : FormalMultilinearSeries ℂ ℂ ℂ}
    (hα : 1 < α) (hC : C ≠ 0) (hR : 1 < R) (hφ0 : 0 < φ) (hφ2 : φ < Real.pi / 2)
    (k : ℕ) (hk : 1 ≤ k)
    (hp : HasFPowerSeriesAt f p 0)
  ...
  have hscale_bigO : (fun n : ℕ => (n : ℝ) ^ (α - 1))
      =O[atTop] (fun n : ℕ => C * (((n : ℝ) ^ (α - 1) / Real.Gamma α * (Real.log n) ^ k : ℝ) : ℂ)) := by
    have habs := algebraic_scale_isLittleO_logK_scale (α := α) hk
    have hpos : (fun n : ℕ => (n : ℝ) ^ (α - 1) * (Real.log n) ^ k)
        =O[atTop] (fun n : ℕ => C * (((n : ℝ) ^ (α - 1) / Real.Gamma α * (Real.log n) ^ k : ℝ) : ℂ)) := by
      rw [Asymptotics.isBigO_iff]
      refine ⟨Real.Gamma α / ‖C‖, ?_⟩
      filter_upwards [eventually_ge_atTop 1] with n hn
