...
namespace AnalyticCombinatorics.PartA.Ch3.Perspective

/-- Coarse coverage score for Chapter III navigation. -/
...
def PerspectiveWindow.controlled (w : PerspectiveWindow) : Prop :=
  0 < w.multivariateModels ∧
    w.parameterModels ≤ w.multivariateModels + w.slack

def PerspectiveWindow.budgetControlled (w : PerspectiveWindow) : Prop :=
  w.certificateBudgetWindow ≤
    w.multivariateModels + w.parameterModels + w.slack
...
end AnalyticCombinatorics.PartA.Ch3.Perspective
