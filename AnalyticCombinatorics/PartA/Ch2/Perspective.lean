...
namespace AnalyticCombinatorics.PartA.Ch2.Perspective

/-- Coarse coverage score for Chapter II navigation. -/
...
def PerspectiveWindow.controlled (w : PerspectiveWindow) : Prop :=
  0 < w.labelledClasses ∧
    w.speciesConstructions ≤ w.labelledClasses + w.slack

def PerspectiveWindow.budgetControlled (w : PerspectiveWindow) : Prop :=
  w.certificateBudgetWindow ≤
    w.labelledClasses + w.speciesConstructions + w.slack
...
end AnalyticCombinatorics.PartA.Ch2.Perspective
