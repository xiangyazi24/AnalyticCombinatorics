...
namespace AnalyticCombinatorics.PartA.Ch1.Perspective

/-- Coarse coverage score for Chapter I navigation. -/
...
def PerspectiveWindow.controlled (w : PerspectiveWindow) : Prop :=
  0 < w.symbolicMethods ∧
    w.constructionExamples ≤ w.symbolicMethods + w.slack

def PerspectiveWindow.budgetControlled (w : PerspectiveWindow) : Prop :=
  w.certificateBudgetWindow ≤
    w.symbolicMethods + w.constructionExamples + w.slack
...
end AnalyticCombinatorics.PartA.Ch1.Perspective
