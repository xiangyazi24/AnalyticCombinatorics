...
namespace AnalyticCombinatorics.PartB.Ch7.Perspective

/-- Coarse coverage score for Chapter VII navigation. -/
...
def PerspectiveWindow.controlled (w : PerspectiveWindow) : Prop :=
  0 < w.implicitSchemas ∧
    w.algebraicApplications ≤ w.implicitSchemas + w.slack

def PerspectiveWindow.budgetControlled (w : PerspectiveWindow) : Prop :=
  w.certificateBudgetWindow ≤
    w.implicitSchemas + w.algebraicApplications + w.slack
...
end AnalyticCombinatorics.PartB.Ch7.Perspective
