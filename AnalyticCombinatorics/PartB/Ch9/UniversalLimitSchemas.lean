...
/-- Quasi-power schema statement. -/
def QuasiPowerSchema
    (_family : ℕ → FinitePMF) (_mean _variance : ℕ → ℚ) : Prop :=
  True
...
/-- Statement form for Airy limit laws in tree parameters. -/
def AiryLimitLaw
    (_family : ℕ → FinitePMF) (_schema : AirySchema) : Prop :=
  True

theorem airy_limit_law_statement
    (family : ℕ → FinitePMF) :
    AiryLimitLaw family treeAirySchema := by
  trivial
