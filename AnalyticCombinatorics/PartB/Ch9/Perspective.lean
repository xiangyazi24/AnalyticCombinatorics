...
namespace AnalyticCombinatorics.PartB.Ch9.Perspective

/-- Coarse coverage score for Chapter IX navigation. -/
...
def PerspectiveWindow.controlled (w : PerspectiveWindow) : Prop :=
  0 < w.limitLawSchemas ∧
    w.multivariateSchemas ≤ w.limitLawSchemas + w.slack

def PerspectiveWindow.budgetControlled (w : PerspectiveWindow) : Prop :=
  w.certificateBudgetWindow ≤
    w.limitLawSchemas + w.multivariateSchemas + w.slack
...
end AnalyticCombinatorics.PartB.Ch9.Perspective
