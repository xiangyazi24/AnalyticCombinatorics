...
namespace AnalyticCombinatorics.PartB.Ch6.Perspective

/-- Coarse coverage score for Chapter VI navigation. -/
...
def PerspectiveWindow.controlled (w : PerspectiveWindow) : Prop :=
  0 < w.transferTheorems ∧
    w.singularSchemas ≤ w.transferTheorems + w.slack

def PerspectiveWindow.budgetControlled (w : PerspectiveWindow) : Prop :=
  w.certificateBudgetWindow ≤
    w.transferTheorems + w.singularSchemas + w.slack
...
end AnalyticCombinatorics.PartB.Ch6.Perspective
