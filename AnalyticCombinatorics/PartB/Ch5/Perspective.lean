...
namespace AnalyticCombinatorics.PartB.Ch5.Perspective

/-- Coarse coverage score for Chapter V navigation. -/
...
structure PerspectiveWindow where
  meromorphicSchemas : ℕ
  automataApplications : ℕ
  certificateBudgetWindow : ℕ
  slack : ℕ
deriving DecidableEq, Repr

def PerspectiveWindow.controlled (w : PerspectiveWindow) : Prop :=
  0 < w.meromorphicSchemas ∧
    w.automataApplications ≤ w.meromorphicSchemas + w.slack

def PerspectiveWindow.budgetControlled (w : PerspectiveWindow) : Prop :=
  w.certificateBudgetWindow ≤
    w.meromorphicSchemas + w.automataApplications + w.slack
...
def samplePerspectiveWindow : PerspectiveWindow :=
  { meromorphicSchemas := 2
    automataApplications := 3
...
end AnalyticCombinatorics.PartB.Ch5.Perspective
