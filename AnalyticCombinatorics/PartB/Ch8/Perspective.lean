...
namespace AnalyticCombinatorics.PartB.Ch8.Perspective

/-- Coarse coverage score for Chapter VIII navigation. -/
...
structure PerspectiveWindow where
  saddlePointMethods : ℕ
  admissibilitySchemas : ℕ
  certificateBudgetWindow : ℕ
  slack : ℕ
deriving DecidableEq, Repr

def PerspectiveWindow.controlled (w : PerspectiveWindow) : Prop :=
  0 < w.saddlePointMethods ∧
    w.admissibilitySchemas ≤ w.saddlePointMethods + w.slack

def PerspectiveWindow.budgetControlled (w : PerspectiveWindow) : Prop :=
  w.certificateBudgetWindow ≤
    w.saddlePointMethods + w.admissibilitySchemas + w.slack
...
def samplePerspectiveWindow : PerspectiveWindow :=
  { saddlePointMethods := 2
    admissibilitySchemas := 3
...
end AnalyticCombinatorics.PartB.Ch8.Perspective
